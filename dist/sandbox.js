...
var sandbox = function () {
	function sandbox(files, options) {
		var _this = this;
		...
		window.requestFileSystem(window.TEMPORARY, 0, function (fs) {
			...
			_this.tree = {};
			if (files.length > 0) {
				_this.readDir('/', function (sandboxFiles) {
					var index = 0;
					...
					(function step(that) {
						if (!sandboxFiles.includes(that.getHash(files[index]))) {
							that.createFileByPath({
								path: files[index],
								callback: function callback(hash, url) {
									that.tree[hash] = url;
									index++;
									if (index == max) {
										//缓存完列表
							...
							that.getUrl(files[index], function (url) {
	...
	createClass(sandbox, [{
		...
		value: function getUrl(path) {
			...
			this.root.getFile(this.getHash(path), {}, function (fileEntry) {
		...
		key: 'createFileByPath',
		value: function createFileByPath(options) {
			var _this2 = this;
			...
			xhr.onload = function () {
				var hash = _this2.getHash(options['path']);

				_this2.root.getFile(hash, {
					create: true
				}, function (fileEntry) {
		...
		value: function readFile(path) {
			...
			this.root.getFile(path, {}, function (fileEntry) {
		...
		key: 'createFile',
		value: function createFile(path, contents) {
			var callback = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : new Function();

			this.root.getFile(path, { create: true }, function (fileEntry) {
				fileEntry.createWriter(function (fileWriter) {
					var truncated = false;
					fileWriter.onwriteend = function () {
						if (!truncated) {
		...
		value: function readDir(path) {
			...
			this.root.getDirectory(path, {}, function (dirEntry) {
		...
		value: function createDir(path) {
			...
			this.root.getDirectory(path, { create: true }, function () {
				callback();
			}, callback);
		}
	}, {
		...
		value: function copy(src, dest) {
			...
			this.root.getFile(src, {}, function (fileEntry) {
				_this3.root.getDirectory(dest, {}, function (dirEntry) {
		...
		key: 'move',
		value: function move(src, dest, callback) {
			var _this4 = this;

			this.root.getFile(src, {}, function (fileEntry) {
				_this4.root.getDirectory(dest, {}, function (dirEntry) {
